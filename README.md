...
- A public idea submission portal with search, Employee ID plus edit access key unlock, HTML/plain-text preview, image attachments, cycle-aware closure, category limits, and pagination.
...
`GirtDemo123!` is only the seeded demo login password for protected core accounts. It is not an idea edit access key. New idea submissions start with an empty edit access key field, and the submitter creates their own private key during submission.
...
- A user can submit during the configured cycle window, receive a unique GRIT submission ID, edit with submitter Employee ID plus private edit access key, and view after expiry.
...
- New submissions require a private edit access key. The key is never stored as plaintext; only a hash is saved.
...
- Only the submitter can edit by using the same browser session, or by unlocking edit access with submitter Employee ID plus the private edit access key.
- Core committee can open `/core/ideas/<idea_id>` from the dashboard/archive to see the saved internal edit token and reset the private edit access key. The existing key is never shown because only its hash is stored.
